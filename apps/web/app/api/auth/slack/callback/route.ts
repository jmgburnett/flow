...
export async function GET(request: NextRequest) {
	if (!SLACK_CLIENT_ID || !SLACK_CLIENT_SECRET || !SLACK_REDIRECT_URI || !CONVEX_URL) {
	...
	if (error) {
		return redirect(request, `/dashboard/settings?error=slack_${encodeURIComponent(error)}`);
	}
	...
	try {
		...
		if (!tokenData.ok) {
			...
			return redirect(request, `/dashboard/settings?error=slack_token_${tokenData.error}`);
		}
