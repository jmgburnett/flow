...
export default function OKRsPage() {
	...
	const [showCreateKR, setShowCreateKR] = useState<Id<"objectives"> | null>(null);
	...
	function resetObjForm() {
		setObjTitle(""); setObjDesc(""); setObjStart(""); setObjEnd("");
	}

	function resetKRForm() {
		setKrTitle(""); setKrTarget(""); setKrUnit(""); setKrOwnerName("");
	}

	async function handleCreateObj() {
		if (!objTitle.trim()) return;
		const start = objStart ? new Date(objStart).getTime() : Date.now();
		const end = objEnd ? new Date(objEnd).getTime() : Date.now() + 90 * 24 * 60 * 60 * 1000;
	...
	async function handleUpdateRAG(objId: Id<"objectives">, rag: "green" | "amber" | "red" | "not_started") {
		await updateObjective({ id: objId, ragStatus: rag });
	}

	async function handleUpdateKRProgress(krId: Id<"key_results">, current: number) {
	...
	const objectives = statusFilter === "active"
		? dashboard?.objectives
		: allObjectives?.map((obj) => ({ ...obj, keyResults: [] as any[], progress: 0 }));
		...
		<DashboardLayout user={user}>
			<div className="p-4 md:p-6 space-y-4 max-w-2xl mx-auto">
				...
				<div className="flex items-center justify-between">
					...
					<Button size="sm" onClick={() => setShowCreateObj(true)} className="gap-1 rounded-xl">
				...
				{dashboard && (
					<div className="grid grid-cols-3 gap-3">
						<div className="glass-card rounded-2xl p-3 text-center">
							<div className="text-xl font-bold text-primary">{dashboard.overallProgress}%</div>
						...
						<div className="glass-card rounded-2xl p-3 text-center">
							<div className="text-xl font-bold text-emerald-500">{dashboard.completedCount}</div>
				...
				<div className="space-y-3">
					{!objectives ? (
						...
						<div className="text-center py-12 text-muted-foreground">
							...
							<p className="text-sm mt-1">Create your first objective to get started</p>
						</div>
					) : (
						objectives.map((obj: any) => {
							const isExpanded = expandedObj.has(obj._id);
							const ragColor = RAG_COLORS[obj.ragStatus as keyof typeof RAG_COLORS];
							const ragLabel = RAG_LABELS[obj.ragStatus as keyof typeof RAG_LABELS];

							return (
								<div key={obj._id} className="glass-card rounded-2xl overflow-hidden">
									{/* Objective header */}
									<button
										type="button"
									...
									>
										<div className="flex items-start gap-3">
											<div className={cn("h-3 w-3 rounded-full mt-1 shrink-0", ragColor)} title={ragLabel} />
											<div className="flex-1 min-w-0">
												...
												<div className="mt-2 flex items-center gap-2">
													...
													<span className="text-[10px] text-muted-foreground shrink-0">{obj.progress ?? 0}%</span>
												</div>
											</div>
											{isExpanded ? (
									...
									{isExpanded && (
										<div className="px-4 pb-4 border-t border-border/50">
											...
											<div className="flex items-center gap-2 py-3">
												...
												{(["green", "amber", "red", "not_started"] as const).map((rag) => (
													<button
														key={rag}
														type="button"
														...
														className={cn(
															...
															RAG_COLORS[rag],
															obj.ragStatus === rag ? "border-foreground scale-110" : "border-transparent opacity-50 hover:opacity-80",
														)}
														title={RAG_LABELS[rag]}
													/>
												))}
												<div className="flex-1" />
												<Button size="sm" variant="ghost" className="text-xs h-7 text-muted-foreground" onClick={() => handleCompleteObj(obj._id)}>
													Complete
												</Button>
												<Button size="sm" variant="ghost" className="text-xs h-7 text-red-500" onClick={() => handleDeleteObj(obj._id)}>
											...
											<div className="space-y-2">
												{(obj.keyResults ?? []).map((kr: any) => {
													...
													return (
														<div key={kr._id} className="bg-muted/30 rounded-xl p-3">
															<div className="flex items-start gap-2 mb-2">
																<div className="flex-1 min-w-0">
																	<p className="text-sm font-medium">{kr.title}</p>
																	{kr.ownerName && (
																		<p className="text-[10px] text-muted-foreground">Owner: {kr.ownerName}</p>
																	)}
																</div>
																<span className={cn("text-xs font-medium", KR_STATUS_COLORS[kr.status as keyof typeof KR_STATUS_COLORS])}>
															...
															<div className="flex items-center gap-2">
																...
																<input
																	...
																	onChange={(e) => handleUpdateKRProgress(kr._id, Number(e.target.value))}
																	className="w-16 h-6 text-xs text-center rounded border bg-background"
																	onClick={(e) => e.stopPropagation()}
																/>
																<Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-red-500" onClick={() => deleteKeyResult({ id: kr._id })}>
																	<Trash2 className="h-3 w-3" />
																</Button>
															</div>
														</div>
			...
			<Dialog open={showCreateObj} onOpenChange={(open) => { if (!open) { resetObjForm(); setShowCreateObj(false); } }}>
				<DialogContent className="sm:max-w-md">
					...
					<div className="space-y-4 py-2">
						<div>
							...
							<Input value={objTitle} onChange={(e) => setObjTitle(e.target.value)} placeholder="e.g. Increase user retention" autoFocus />
						</div>
						<div>
							<label className="text-sm font-medium mb-1.5 block">Description</label>
							<Textarea value={objDesc} onChange={(e) => setObjDesc(e.target.value)} placeholder="What does success look like?" rows={2} className="resize-none" />
						</div>
						<div className="grid grid-cols-2 gap-3">
							<div>
								<label className="text-sm font-medium mb-1.5 block">Start Date</label>
								<Input type="date" value={objStart} onChange={(e) => setObjStart(e.target.value)} />
							</div>
							<div>
								<label className="text-sm font-medium mb-1.5 block">End Date</label>
								<Input type="date" value={objEnd} onChange={(e) => setObjEnd(e.target.value)} />
							</div>
						</div>
					</div>
					<DialogFooter>
						<Button variant="outline" onClick={() => { resetObjForm(); setShowCreateObj(false); }}>Cancel</Button>
						<Button onClick={handleCreateObj} disabled={!objTitle.trim()}>Create</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>

			{/* Create Key Result Dialog */}
			<Dialog open={showCreateKR !== null} onOpenChange={(open) => { if (!open) { resetKRForm(); setShowCreateKR(null); } }}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2">
					...
					<div className="space-y-4 py-2">
						<div>
							...
							<Input value={krTitle} onChange={(e) => setKrTitle(e.target.value)} placeholder="e.g. Achieve 50% monthly active retention" autoFocus />
						</div>
						<div className="grid grid-cols-2 gap-3">
							<div>
							...
							<div>
								...
								<Input value={krUnit} onChange={(e) => setKrUnit(e.target.value)} placeholder="e.g. %, count, $" />
							</div>
						</div>
						<div>
							<label className="text-sm font-medium mb-1.5 block">Owner Name</label>
							<Input value={krOwnerName} onChange={(e) => setKrOwnerName(e.target.value)} placeholder="Who's responsible?" />
						</div>
					</div>
					<DialogFooter>
						<Button variant="outline" onClick={() => { resetKRForm(); setShowCreateKR(null); }}>Cancel</Button>
						<Button onClick={handleCreateKR} disabled={!krTitle.trim() || !krTarget}>Create</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</DashboardLayout>
