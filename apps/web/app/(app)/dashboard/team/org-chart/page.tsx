...
import { Network, ChevronLeft, Users, ChevronDown, ChevronRight as ChevronRightIcon } from "lucide-react";
				...
				<div className="flex items-center gap-3">
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-center gap-2">
							...
							{node.department && (
								<Badge variant="secondary" className="text-[10px] shrink-0">{node.department}</Badge>
							)}
						</div>
						{node.role && (
...
export default function OrgChartPage() {
	...
	const departments = [
		...new Set(teamMembers?.map((m) => m.department).filter(Boolean) as string[]),
	];
			...
			<div className="p-4 md:p-6 space-y-4 max-w-2xl mx-auto">
					...
					<div className="flex flex-wrap gap-2">
						{departments.map((dept) => (
							<div key={dept} className="flex items-center gap-1.5 text-xs text-muted-foreground">
								<div className={cn("h-2.5 w-0.5 rounded-full", getDeptColor(dept).replace("border-l-", "bg-"))} />
								<span>{dept}</span>
							</div>
						))}
					</div>
				...
				{!orgChart ? (
					...
					<div className="text-center py-12 text-muted-foreground">
						...
						<p className="text-sm mt-1">Set reporting relationships on team member profiles to build the org chart</p>
					</div>
				) : (
					<div className="space-y-0">
						{orgChart.map((root) => (
