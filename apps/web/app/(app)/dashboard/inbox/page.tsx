...
export default function InboxPage() {
  ...
  if (selectedEmail) {
    const triage =
      TRIAGE_CONFIG[selectedEmail.triageStatus as keyof typeof TRIAGE_CONFIG];
    return (
        ...
        <div className="max-w-2xl mx-auto w-full p-4 md:p-6 space-y-4 pb-40">
          ...
          <div className="space-y-2">
            <div className="flex items-start justify-between gap-2">
              <h2 className="font-semibold text-lg leading-tight tracking-tight flex-1">
                {selectedEmail.subject}
              </h2>
              {triage && (
                <Badge
                  className={cn(
                    "text-[10px] shrink-0 rounded-full",
                    triage.color,
                  )}
                >
                  {triage.label}
            ...
            <div className="text-sm text-muted-foreground space-y-0.5">
              ...
              <p className="text-xs">
                to {selectedEmail.to?.join(", ")} · {selectedEmail.accountEmail}{" "}
                · {fmtDate(selectedEmail.receivedAt)}
              </p>
            </div>
          </div>
              ...
              {!showReply && !sendSuccess ? (
                <>
                  <Button
                    ...
                    className="rounded-xl gap-1.5 flex-1 md:flex-none"
                  >
        ...
        <div className="space-y-2">
          {emails.length === 0 ? (
            ...
            emails.map((email: any) => {
              const triage =
                TRIAGE_CONFIG[email.triageStatus as keyof typeof TRIAGE_CONFIG];
