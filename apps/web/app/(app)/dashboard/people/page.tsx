...
export default function PeoplePage() {
  ...
  const mergedPeople = (() => {
    ...
    for (const profile of profiles) {
      ...
      people.push({
        ...
        interactionCount:
          (profile.emailsSent || 0) +
          (profile.emailsReceived || 0) +
          (profile.sharedMeetings || 0),
      });
    }
    ...
    if (contacts) {
      for (const contact of contacts) {
        const hasProfile = contact.emails?.some((e: string) =>
  ...
  function inferTypeFromEmail(email: string): ContactType {
    if (email.endsWith("@gloo.us") || email.endsWith("@gloo.tech"))
      return "coworker";
    if (email.endsWith("@church.tech") || email.endsWith("@onflourish.com"))
      return "team_member";
    return "contact";
  }

  // Filter + search
  const filteredContacts = mergedPeople
    ?.filter((c: any) => {
      // Type filter
    ...
    ?.sort(
      (a: any, b: any) => (b.interactionCount || 0) - (a.interactionCount || 0),
    );
          ...
          {profile ? (
              ...
              {profile.recentInteractions?.length > 0 && (
                <div className="glass-card rounded-2xl p-5 space-y-3">
                  ...
                  <div className="space-y-2">
                    {profile.recentInteractions.map(
            ...
            <div className="glass-card rounded-2xl p-5 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Profiles are built automatically from email, calendar, and
                messages
              </p>
            </div>
          )}
          ...
          <div className="flex gap-2">
            {(pendingCount ?? 0) > 0 && (
              <Button
                ...
                className="gap-1.5 relative rounded-xl"
              >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {pending.matchedContactId && (
                        <div className="mt-1.5 flex items-center gap-1.5 text-xs text-yellow-600 dark:text-yellow-400">
                          <GitMerge className="h-3 w-3" /> Possible match:{" "}
                          {pending.matchReason}
                    ...
                    {pending.matchedContactId ? (
                      <>
                        <Button
                          ...
                          onClick={() =>
                            handleMerge(pending._id, pending.matchedContactId)
                          }
                        >
                          ...
                          <>
                            {contact.personality &&
                              (contact.personality.mbti ||
                                ...
                                <div className="flex gap-1 mt-1.5 overflow-hidden">
                                  ...
                                  {contact.personality.enneagram && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400">
                                      {contact.personality.enneagram}
                                    </span>
                                  )}
                                  {contact.personality.disc && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">
                                      DISC: {contact.personality.disc}
                                    </span>
                                  )}
                                </div>
                              )}
                            {contact.skills && contact.skills.length > 0 && (
                              <div className="flex items-center gap-1 mt-1">
                                <span className="text-[10px] text-muted-foreground">
                                  🎯 {contact.skills.length} skill
                                  {contact.skills.length !== 1 ? "s" : ""}
                                </span>
                                {contact.department && (
                                  <span className="text-[10px] text-muted-foreground">
                                    · 🏢 {contact.department}
                        ...
                        <div className="flex items-center gap-3 mt-1.5 text-[10px] text-muted-foreground">
                          ...
                          {(profile?.sharedMeetings ?? 0) > 0 && (
                            <span className="flex items-center gap-0.5">
                              ...
                              {profile.sharedMeetings}
                            </span>
                          )}
                          {(profile?.sources ?? contact.sources)?.map(
                            (s: string) => (
                              <span key={s} title={s}>
                                {SOURCE_ICONS[s]}
                              </span>
