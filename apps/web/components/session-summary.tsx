...
export function SessionSummaryPanel({ sessionId }: { sessionId: Id<"capture_sessions"> }) {
	...
	return (
		<div className="flex flex-col h-full">
			...
			<div className="flex-1 overflow-y-auto px-3 py-3">
					...
					<div className="space-y-2">
						{summary.actionItems.length === 0 ? (
							...
							summary.actionItems.map((item, i) => {
								const urgency = URGENCY_CONFIG[item.urgency as keyof typeof URGENCY_CONFIG] ?? URGENCY_CONFIG.medium;
								return (
									<div key={i} className="flex items-start gap-2.5 p-2.5 glass-card rounded-xl">
										...
										<div className="flex items-center gap-1 shrink-0 mt-0.5">
											<div className={cn("w-1.5 h-1.5 rounded-full", urgency.dot)} title={urgency.label} />
										</div>
									</div>
								);
							})
				...
				{tab === "people" && (
					<div className="space-y-2">
						{!summary.peopleMentioned || summary.peopleMentioned.length === 0 ? (
							<div className="flex flex-col items-center gap-2 py-6 text-center">
