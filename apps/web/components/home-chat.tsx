...
import { Send, Loader2, Sparkles, Plus, MessageSquare, Trash2, ChevronLeft } from "lucide-react";
...
function ChatHistory({
	...
	const conversations = useQuery(api.chat.listConversations, { userId: "josh", limit: 30 });
					...
					conversations.map((convo) => (
						...
						>
							...
							>
								...
								<p className="text-[10px] text-muted-foreground">{fmtDate(convo.updatedAt)}</p>
							</button>
							<button
								type="button"
								onClick={(e) => {
...
export function HomeChat() {
	...
	const [activeConversationId, setActiveConversationId] = useState<Id<"chat_conversations"> | null>(null);
	...
	const handleSend = async (text?: string) => {
		...
		try {
			...
			if (!activeConversationId && result.conversationId) {
				setActiveConversationId(result.conversationId as Id<"chat_conversations">);
			}
		} catch (error) {
	...
	const hasMessages = activeConversationId && storedMessages && storedMessages.length > 0;
		...
		<div className="flex h-full">
			...
			<div
				className={cn(
					...
					// Mobile: overlay
					showHistory ? "fixed inset-0 z-40 w-[280px] glass-heavy md:relative md:z-auto" : "hidden",
					// Desktop: always show
					"md:block md:w-[240px]",
				)}
			>
