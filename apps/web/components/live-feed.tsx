...
const CATEGORY_CONFIG = {
	task: { icon: Zap, label: "Task", color: "text-blue-500", bg: "bg-blue-500/10" },
	commitment: { icon: Clock, label: "Commitment", color: "text-orange-500", bg: "bg-orange-500/10" },
	decision: { icon: GitBranch, label: "Decision", color: "text-green-500", bg: "bg-green-500/10" },
	follow_up: { icon: MessageCircle, label: "Follow-up", color: "text-purple-500", bg: "bg-purple-500/10" },
	question: { icon: HelpCircle, label: "Question", color: "text-yellow-500", bg: "bg-yellow-500/10" },
};
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-2 mb-1.5">
				<div className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-medium", cat.bg, cat.color)}>
				...
				<div className={cn("w-1.5 h-1.5 rounded-full", urg.dot)} title={urg.label} />
				<span className="text-[10px] text-muted-foreground">{time}</span>
			</div>

			{/* Description */}
			<p className="text-sm font-medium leading-snug mb-1">{task.description}</p>
			...
			{task.status === "approved" && (
				<span className="text-[10px] font-medium text-green-600">✓ Approved</span>
			)}
			{task.status === "converted" && (
				<span className="text-[10px] font-medium text-primary">→ Task created</span>
			)}
			{task.status === "dismissed" && (
				<span className="text-[10px] font-medium text-muted-foreground">✕ Dismissed</span>
			)}
		</div>
	);
}
		...
		<div className="flex flex-col h-full">
			...
			<div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
				{!tasks || tasks.length === 0 ? (
					...
					tasks.map((task: any) => (
						<LiveTaskCard key={task._id} task={task} />
					))
				)}
			</div>
		</div>
