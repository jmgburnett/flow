...
function isObviouslyJunk(email: string): { junk: boolean; reason: string } {
	...
	if (domain) {
		for (const pattern of JUNK_DOMAIN_PATTERNS) {
			if (pattern.test(domain)) {
				return { junk: true, reason: `Domain matches automated service: ${domain}` };
			}
		}
	}
	...
	if (localPart && localPart.length > 40) {
		return { junk: true, reason: "Email local part looks like tracking/bounce ID" };
	}

	// Contains + addressing with long suffix (tracking)
	if (localPart && localPart.includes("+") && localPart.split("+")[1].length > 15) {
...
JSON only, no markdown:`,
				...
				try {
					...
					for (const profile of batch) {
						await ctx.runMutation(internal.profileBuilder.patchProfile, {
							profileId: profile._id,
							patch: { isReal: true, filterReason: "classification_parse_error", updatedAt: Date.now() },
						});
						kept++;
					}
					continue;
				...
				for (const classification of classifications) {
					...
					if (classification.real) {
						await ctx.runMutation(internal.profileBuilder.patchProfile, {
							profileId: profile._id,
							patch: { isReal: true, filterReason: classification.reason, updatedAt: Date.now() },
						});
						kept++;
					} else {
		...
		for (const profile of profiles) {
			...
			if (inbound) {
				...
				const alreadyHasInbound = interactions.some(
					(i) => i.type === "email_received" && i.summary === inbound.latestSubject,
				);
