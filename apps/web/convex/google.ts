    ...
    for (const event of events) {
      ...
      for (const attendee of event.attendees || []) {
        ...
        const attendeeName: string =
          attendee.displayName || attendeeEmail.split("@")[0];
        try {
          await ctx.runMutation(internal.people.extractPerson, {
            userId: connection.userId,
            name: attendeeName,
...
export const sendReply = action({
  ...
  handler: async (ctx, args) => {
    ...
    const connection = await ctx.runQuery(
      internal.google.getConnectionByEmail,
      {
...
No emojis, no corporate jargon, no "Best regards". Match length to content. Writes like talking to a friend he respects.`;
    ...
    const senderEmail = senderEmailMatch
      ? senderEmailMatch[1].toLowerCase()
      : email.from.toLowerCase();
    let recipientContext = "";
    try {
      const recipientProfile = await ctx.runQuery(
        internal.profileBuilder.getProfileByEmailInternal,
        {
          userId: email.userId,
          email: senderEmail,
        },
