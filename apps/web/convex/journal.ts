...
export const setUserPreferences = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("user_preferences", { userId, ...args });
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      for (const mc of meetingContexts) {
        const time = new Date(mc.startedAt).toLocaleTimeString("en-US", {
          hour: "numeric",
          minute: "2-digit",
        });
      ...
      if (calendarEvents) {
        for (const evt of calendarEvents) {
          ...
          const endTime = new Date(evt.endTime).toLocaleTimeString("en-US", {
            hour: "numeric",
            minute: "2-digit",
          });
          const attendeeList =
            evt.attendees?.join(", ") ?? "no attendees listed";
