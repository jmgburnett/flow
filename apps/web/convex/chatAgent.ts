...
const TOOLS: Tool[] = [
	{
		...
		input_schema: {
			...
			properties: {
				timeMin: {
					type: "string",
					description: "Start of time range in ISO 8601 format (e.g. 2026-03-15T09:00:00-05:00)",
				},
				timeMax: {
					type: "string",
				...
				attendeeEmails: {
					type: "array",
					items: { type: "string" },
					description: "Email addresses to check availability for. Always include Josh's calendars.",
				},
			},
		...
		input_schema: {
			...
			properties: {
				attendeeEmails: {
					type: "array",
					items: { type: "string" },
					description: "Email addresses of people to find mutual availability with",
				},
				durationMinutes: {
					type: "number",
	...
	},
	{
		...
		input_schema: {
			...
			properties: {
				...
				startTime: {
					type: "string",
					description: "Event start time in ISO 8601 format with timezone (e.g. 2026-03-15T10:00:00-05:00)",
				},
				endTime: {
					type: "string",
				...
				calendarAccountEmail: {
					type: "string",
					description: "Which of Josh's Google accounts to create the event on. Default: josh@onflourish.com",
				},
			},
...
export const getUpcomingEvents = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		return events
			.filter((e) => e.startTime >= args.startTime && e.startTime <= args.endTime)
...
async function handleCheckAvailability(
	...
	for (const [calId, calData] of Object.entries(data.calendars || {})) {
		const cal = calData as { busy?: Array<{ start: string; end: string }>; errors?: unknown[] };
		if (cal.errors && cal.errors.length > 0) {
			result[calId] = { status: "unavailable", note: "Cannot access this calendar (likely external)" };
...
async function handleFindOpenSlots(
	...
	let currentDate = new Date(`${startDate}T${String(preferredStartHour).padStart(2, "0")}:00:00${offsetStr}`);
	const searchEnd = new Date(`${endDate}T${String(preferredEndHour).padStart(2, "0")}:00:00${offsetStr}`);
	...
	return JSON.stringify({
		...
		note: unavailableCalendars.length > 0
...
async function handleCreateEvent(
	...
	const connection = connections.find((c: any) => c.email === calendarAccountEmail);
	if (!connection) {
		return JSON.stringify({
