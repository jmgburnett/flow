...
export const list = query({
	...
	handler: async (ctx, args) => {
		...
		return contacts.sort((a, b) => (b.lastInteraction ?? 0) - (a.lastInteraction ?? 0));
	},
});
...
export const extractPerson = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.email && /no-?reply|mailer-daemon|notifications?@/i.test(args.email)) {
		...
		let match: { contactId: typeof contacts[0]["_id"]; reason: string; confidence: number } | null = null;

		for (const contact of contacts) {
			// Exact email match → high confidence
			if (args.email && contact.emails?.includes(args.email)) {
				match = { contactId: contact._id, reason: "Same email address", confidence: 0.95 };
				break;
			}
			// Exact phone match → high confidence
			if (args.phone && contact.phones?.includes(args.phone)) {
				match = { contactId: contact._id, reason: "Same phone number", confidence: 0.95 };
				break;
			}
			// Exact name match
			if (contact.name.toLowerCase() === args.name.toLowerCase()) {
				match = { contactId: contact._id, reason: "Same name", confidence: 0.8 };
				break;
			}
			// Partial name match
			...
			if (overlap.length > 0 && nameParts.length > 1) {
				match = { contactId: contact._id, reason: `Shared name: "${overlap.join(" ")}"`, confidence: 0.5 };
		...
		for (const p of existingPending) {
			...
			if (p.name.toLowerCase() === args.name.toLowerCase()) return "already_pending";
		}
