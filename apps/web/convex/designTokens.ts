...
export const getTokens = query({
  args: {},
  handler: async (ctx) => {
    const tokens = await ctx.db.query("design_tokens").order("desc").first();
    return tokens;
  },
});
  ...
  for (const fill of node.fills || []) {
    if (fill.type === "SOLID" && fill.visible !== false) {
      ...
      if (
        r !== undefined &&
        name &&
        !name.match(/^(Frame|Rectangle|BG|Group|Vector|Ellipse|Line|\d)/i)
      ) {
        results.push({
          name: currentPath,
          hex: rgbaToHex(r, g, b),
...
function extractMaterials(node: any, results: any[] = []): any[] {
  ...
  for (const eff of node.effects || []) {
    ...
    if (eff.type === "DROP_SHADOW" || eff.type === "INNER_SHADOW") {
      ...
      shadows.push(
        `${prefix}${eff.offset?.x || 0}px ${eff.offset?.y || 0}px ${eff.radius || 0}px ${eff.spread || 0}px rgba(${Math.round((c.r || 0) * 255)}, ${Math.round((c.g || 0) * 255)}, ${Math.round((c.b || 0) * 255)}, ${(c.a || 0).toFixed(2)})`,
      );
...
function buildCSS(
  ...
  for (const c of colors) {
    ...
    const meaningful = parts
      .filter(
        (p: string) =>
          !p.match(/^(Color|Colors|Swatch|Style|Component|Instance|Frame|\d)/i),
      )
      .slice(-2)
      .join("-");
    if (!meaningful || meaningful.length < 2) continue;
    const varName = meaningful
      .toLowerCase()
      .replace(/\s+/g, "-")
      .replace(/[^a-z0-9-]/g, "")
      .replace(/-+/g, "-");
    ...
    if (
      (c.hex === "#ffffff" || c.hex === "#000000") &&
      !varName.match(/page|card|surface|bg/)
    )
      continue;
  ...
  const fontFamilies = [
    ...new Set(uniqueTypo.map((t: any) => t.fontFamily).filter(Boolean)),
  ];
  const fontSizes = [...new Set(uniqueTypo.map((t: any) => t.fontSize))].sort(
...
export const syncFromFigma = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    for (const [_, nd] of Object.entries<any>(colorData.nodes || {}))
      extractColors(nd.document, "", colors);

    const typography: any[] = [];
    for (const [_, nd] of Object.entries<any>(typoData.nodes || {}))
      extractTypography(nd.document, typography);

    const spacingValues: number[] = [];
    for (const [_, nd] of Object.entries<any>(spacingData.nodes || {}))
      extractSpacing(nd.document, spacingValues);

    const materials: any[] = [];
    for (const [_, nd] of Object.entries<any>(materialsData.nodes || {}))
      extractMaterials(nd.document, materials);
    ...
    await ctx.runMutation(internal.designTokens.storeTokens, {
      ...
      typography: [...new Set(typography.map((t: any) => JSON.stringify(t)))]
    ...
    console.log(
      `🎨 Synced ${colors.length} colors, ${typography.length} type styles, ${spacingValues.length} spacing values, ${materials.length} materials`,
    );
  },
});
