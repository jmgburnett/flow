	...
	handler: async (ctx, args) => {
		...
		if (contact.directReports) {
			for (const reportId of contact.directReports) {
				...
				if (report) directReportNames.push({ id: report._id, name: report.name });
			}
		}
...
export const getSkillGaps = query({
	...
	handler: async (ctx, args) => {
		...
		const skillCoverage = new Map<string, { total: number; advanced: number; expert: number }>();
	...
	handler: async (ctx, args) => {
		...
		for (const skill of skills) {
			const hasMember = teamMembers.some(
				(m) => m.skills?.some((s) => s.skillId === skill._id),
			);
		...
		return {
			...
			coveragePercent: skills.length > 0 ? Math.round((coveredCount / skills.length) * 100) : 0,
