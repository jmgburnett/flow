  ...
  for (const fill of fills) {
    if (fill.type === "SOLID" && fill.visible !== false) {
      ...
      if (r !== undefined) {
        ...
        if (
          name &&
          !name.match(/^(Frame|Rectangle|BG|Group|Vector|Ellipse|Line|\d)/)
        ) {
          results.push({
            name: currentPath,
            hex: rgbaToHex(r, g, b),
...
function extractMaterials(
  ...
  for (const eff of effects) {
    ...
    if (eff.type === "DROP_SHADOW" || eff.type === "INNER_SHADOW") {
      ...
      shadows.push(
        `${prefix}${ox}px ${oy}px ${blur}px ${spread}px rgba(${r}, ${g}, ${b}, ${a})`,
      );
...
function buildTokenCSS(
  ...
  const fontFamilies = [
    ...new Set(typography.map((t) => t.fontFamily).filter(Boolean)),
  ];

  // Unique font sizes (sorted desc)
  const fontSizes = [...new Set(typography.map((t) => t.fontSize))].sort(
    (a, b) => b - a,
  );

  // Spacing scale (sorted)
  const spacing = [
    ...new Set(spacingValues.map(Number).filter((n) => !isNaN(n))),
  ].sort((a, b) => a - b);
